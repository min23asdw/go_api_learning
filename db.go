...
func NewMySQLStorage(cfg mysql.Config) *MySQLStorage {
	...
	db, err := sql.Open("mysql", cfg.FormatDSN()) //user:password@/dbname

	if err != nil {
		log.Fatal(err)
	}
	err = db.Ping() //ping for make connection
	if err != nil {
		log.Fatal(err)
	}

	log.Println("Connection to MySQL")
	return &MySQLStorage{db: db}
}

func (s *MySQLStorage) Init() (*sql.DB, error) {
	// initialize the tables
	log.Println("initialize the tables")
